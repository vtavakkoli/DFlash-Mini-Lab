...
def main() -> None:
    ...
    p.add_argument("--top-k", type=int, default=4, help="Top-k candidates used by DFlash2/MOBS/JUMP-MOBS")
    p.add_argument("--mobs-refine-passes", type=int, default=0, help="Optional odd/even local refinement passes for DFlash3-MOBS")
    p.add_argument("--jump-weight", type=float, default=0.5, help="Relative sparse jump-head score weight for DFlash4-JUMP-MOBS")
    args = p.parse_args()
    ...
    payload = run_benchmark(args.weights, args.tokenizer, args.prompts, benchmark_json, args.tokens, args.warmups, args.repeats, args.top_k, args.mobs_refine_passes, args.jump_weight)
