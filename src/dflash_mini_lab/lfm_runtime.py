...
class LfmReferenceRuntime:
    ...
    The target is the unmodified Hugging Face LFM checkpoint. The auxiliary
    condition uses frozen target input embeddings to avoid adding an extra
    target-model forward pass before every speculative block.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.candidate_size = int(self.candidate_ids.size)
        self.block_size = int(config.block_size)
        self.jump_offsets = np.asarray(JUMP_OFFSETS, dtype=np.int64)
        self.drafter = drafter.eval()
        self.selector = selector.eval()
        self.jump = jump.eval()
        self.fused = fused.eval()
        self.selector_a = self.selector.a.weight.detach().float().cpu().numpy()
        self.selector_b = self.selector.b.weight.detach().float().cpu().numpy()
        self.selector_scale = float(self.selector.scale)
        self.fused_scale = float(self.fused.scale)
        ...
        self.target.eval(); self.target.to("cpu")
        ...
        self.aux_parameter_count = sum(
            p.numel() for model in (self.drafter, self.selector, self.jump, self.fused) for p in model.parameters()
        )
    ...
    def target_logits(self, input_ids: np.ndarray) -> np.ndarray:
        ids = torch.from_numpy(np.asarray(input_ids, dtype=np.int64)).long().unsqueeze(0)
    ...
    def context_features(self, input_ids: np.ndarray) -> np.ndarray:
        ids = torch.from_numpy(np.asarray(input_ids, dtype=np.int64)).long()
        emb = self.embedding[ids].float()
        context = torch.cat([emb[-1], emb.mean(dim=0)], dim=-1)
    ...
    def jump_logits(self, context: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        x = torch.from_numpy(np.asarray(context, dtype=np.float32)).unsqueeze(0)
        return self.jump_offsets.copy(), self.jump(x)[0].float().cpu().numpy()

    def proposal_argmax(self, draft_logits: np.ndarray) -> np.ndarray:
        local = np.argmax(draft_logits, axis=-1).astype(np.int64)
        return self.candidate_ids[local]

    def _top_k(self, draft_logits: np.ndarray, top_k: int):
        local, vals = _top_k_local(draft_logits, top_k)
        target = self.candidate_ids[local]
        return local, target, vals

    @torch.inference_mode()
    def _selector_state(self, context: np.ndarray):
        x = torch.from_numpy(np.asarray(context, dtype=np.float32)).unsqueeze(0)
        gate = self.selector.gate(x)[0].float().cpu().numpy()
        return gate, self.selector_a, self.selector_b
    ...
    def dflash2_select_path(self, draft_logits: np.ndarray, context: np.ndarray, prev_token: int, top_k: int = 8) -> np.ndarray:
        ...
        for pos in range(1, top_ids.shape[0]):
            ...
            dp = scores[best, np.arange(scores.shape[1])]
            backptrs.append(best)
    ...
    def dflash3_mobs_select_path(self, draft_logits: np.ndarray, context: np.ndarray, prev_token: int, top_k: int = 8, refine_passes: int = 0):
        ...
        if block % 2:
            ...
            finger = int(float(np.abs(context[: min(8, context.size)]).sum()) * 1_000_000.0)
            anchor = centers[(finger ^ (int(prev_token) * 0x9E3779B1)) & 1]
        chosen = np.full(block, -1, dtype=np.int64)
        c = top_ids[anchor]; s = top_vals[anchor] + forward(prev_token, c)
        chosen[anchor] = int(c[int(np.argmax(s))])
        for dist in range(1, block + 1):
            left = anchor - dist
            if left >= 0:
                c = top_ids[left]; s = top_vals[left] + backward(c, int(chosen[left + 1]))
                if left == 0:
                    s += forward(prev_token, c)
                chosen[left] = int(c[int(np.argmax(s))])
            right = anchor + dist
            if right < block:
                c = top_ids[right]; s = top_vals[right] + forward(int(chosen[right - 1]), c)
                chosen[right] = int(c[int(np.argmax(s))])

        # The real CPU benchmark intentionally keeps the already-established
        # MOBS fast path: zero bubble/refinement passes.
        if refine_passes:
            raise ValueError("LFM real benchmark currently supports refine_passes=0 only")
    ...
    def _anchored_gap_fill(self, top_ids: np.ndarray, top_vals: np.ndarray, chosen: np.ndarray, context: np.ndarray, prev_token: int):
        ...
        while np.any(chosen < 0):
            ...
            for pos in range(block):
                if snapshot[pos] >= 0:
                    continue
                if pos == 0 or snapshot[pos - 1] >= 0 or (pos + 1 < block and snapshot[pos + 1] >= 0):
                    frontier.append(pos)
            if not frontier:
                raise RuntimeError("anchored gap filling could not advance")
            for pos in frontier:
                c = top_ids[pos]; s = top_vals[pos].copy()
                if pos == 0:
                    s += forward(prev_token, c)
                elif snapshot[pos - 1] >= 0:
                    s += forward(int(snapshot[pos - 1]), c)
                if pos + 1 < block and snapshot[pos + 1] >= 0:
                    s += backward(c, int(snapshot[pos + 1]))
    ...
    def dflash4_jump_mobs_select_path(self, draft_logits, jump_offsets, jump_logits, context, prev_token, top_k=8, jump_weight=0.5):
        ...
        for row, offset in enumerate(np.asarray(jump_offsets, dtype=np.int64).tolist()):
            ...
            if pos < 0 or pos >= block or row >= int(jump_logits.shape[0]):
                continue
            local = top_local[pos]; c = top_ids[pos]; count += int(c.size)
            bonus = np.asarray(jump_logits[row, local], dtype=np.float32); bonus -= float(np.max(bonus))
        ...
        chosen, pairs = self._anchored_gap_fill(top_ids, top_vals, chosen, context, prev_token)
        return chosen, int(pairs), count
    ...
    def dflash5_fused_jump_mobs_select_path(self, draft_hidden, draft_logits, context, prev_token, top_k=8, fused_weight=1.0, min_margin=0.0):
        ...
        hidden = torch.from_numpy(np.asarray(draft_hidden, dtype=np.float32)).unsqueeze(0)
        residual = self.fused.residual_logits(hidden)[0].float().cpu().numpy()
        positions = self.jump_offsets - 1
        chosen = np.full(block, -1, dtype=np.int64); anchors = 0; count = 0
        for row, pos in enumerate(positions.tolist()):
            if pos < 0 or pos >= block:
                continue
            local = top_local[pos]; c = top_ids[pos]
            extra = residual[row, local]; count += int(local.size)
            scores = top_vals[pos] + float(fused_weight) * extra
            best = int(np.argmax(scores))
            if float(min_margin) > 0 and scores.size > 1:
                second = float(np.partition(scores, -2)[-2])
                if float(scores[best] - second) < float(min_margin):
                    continue
            chosen[pos] = int(c[best]); anchors += 1
        if anchors == 0:
            fallback, work = self.dflash3_mobs_select_path(draft_logits, context, prev_token, top_k=top_k, refine_passes=0)
            return fallback, int(work), count, 0
        chosen, pairs = self._anchored_gap_fill(top_ids, top_vals, chosen, context, prev_token)
        return chosen, int(pairs), count, anchors
    ...
    def dflash6_boltzmann_select_path(self, draft_logits, context, prev_token, top_k=8, temperature=0.15):
        ...
        for pos in range(top_ids.shape[0]):
            c = top_ids[pos]
            scores = top_vals[pos].astype(np.float64) / temps[pos] + _gumbel_for_ids(c, seed, pos)
            chosen[pos] = int(c[int(np.argmax(scores))])
