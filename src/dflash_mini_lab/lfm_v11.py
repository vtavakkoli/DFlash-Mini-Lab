...
def _alt_probability(margin: float, temperature: float) -> float:
    """Two-way Boltzmann probability mass assigned to the runner-up token."""
    temp = max(1e-5, float(temperature))
    z = min(60.0, max(0.0, float(margin)) / temp)
    return 1.0 / (1.0 + math.exp(z))


def select_v11_boltzmann_gated_mobs(
    runtime,
    ...
    """Use Boltzmann uncertainty only as a cheap gate, then sparse MOBS.

    The default path is plain DFlash argmax. Positions whose top-1/top-2 gap
    carries enough Boltzmann alternative probability compete for a small MOBS
    budget. Only those slots pay pairwise predecessor/successor scoring.

    This is deterministic: Boltzmann supplies an uncertainty probability, not a
    sampling coin. Exact target verification remains authoritative downstream.
    """
    ...
    uncertainty = np.asarray(
        [_alt_probability(float(m), float(config.temperature)) for m in margins],
        dtype=np.float64,
    )

    # Earlier positions have greater prefix value: fixing a late draft token is
    # useless if an earlier one already rejects. The bias is deliberately tiny
    # so uncertainty remains the dominant gate signal.
    priority = uncertainty / (
        1.0 + float(config.early_position_bias) * np.arange(block, dtype=np.float64)
    )
    ...
    # Sparse middle-out refinement. Non-gated slots remain the cheap DFlash
    # argmax anchors; gated slots are visited from the center outward so each
    # correction can use already-stable neighbors without a global DP pass.
    center = (block - 1) / 2.0
...
def config_key(config: V11Config) -> str:
    return (
        f"T={config.temperature:g}|gate={config.gate_floor:g}|budget={config.mobs_budget}"
        f"|k={config.top_k}|pair={config.pair_weight:g}|early={config.early_position_bias:g}"
    )
