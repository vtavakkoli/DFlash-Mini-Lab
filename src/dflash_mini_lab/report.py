...
def build_report(benchmark_json: str | Path, speed_gif: str | Path, architecture_gif: str | Path, out_html: str | Path) -> None:
    ...
    refine_passes = int(data["config"].get("dflash3_mobs_refine_passes", 0))
    refinement_text = (
        f"This run enables {refine_passes} fixed odd/even bubble-like local refinement pass{'es' if refine_passes != 1 else ''}."
        if refine_passes
        else "This run uses the pure CPU fast path with no local refinement; odd/even bubble-like refinement remains available as an optional ablation."
    )
