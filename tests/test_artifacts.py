...
def test_benchmark_and_report_are_generated(tmp_path):
    ...
    payload = run_benchmark(ROOT / "models/tiny_dflash_lab.npz", ROOT / "models/tokenizer.json", ROOT / "benchmarks/prompts.json", data, max_new_tokens=4, warmups=0, repeats=1, top_k=8, mobs_refine_passes=0, jump_weight=0.5)
    for method in ("normal", "dflash", "dflash2", "dflash3_mobs", "dflash4_jump_mobs"):
    ...
    assert "DFlash4-JUMP-MOBS" in text
