...
class WordTokenizer:
    ...
    def from_text(cls, text: str) -> "WordTokenizer":
        ...
        vocab = ["<pad>", "<bos>", "<unk>"] + sorted(set(toks))
        return cls({t: i for i, t in enumerate(vocab)}, vocab)

    def encode(self, text: str, add_bos: bool = True) -> list[int]:
        ids = [self.stoi.get(t, self.stoi["<unk>"]) for t in TOKEN_RE.findall(text.lower())]
        return ([self.stoi["<bos>"]] + ids) if add_bos else ids
...
class TinyTransformerLM(nn.Module):
    def __init__(self, vocab_size: int, d_model: int = 96, nhead: int = 4, layers: int = 2, max_len: int = 256):
        ...
        layer = nn.TransformerEncoderLayer(d_model=d_model, nhead=nhead, dim_feedforward=d_model * 4, dropout=0.0, batch_first=True, norm_first=True)
        self.transformer = nn.TransformerEncoder(layer, num_layers=layers); self.norm = nn.LayerNorm(d_model)
        self.lm_head = nn.Linear(d_model, vocab_size, bias=False); self.lm_head.weight = self.token_emb.weight

    def forward(self, input_ids: torch.Tensor) -> torch.Tensor:
        _, seq_len = input_ids.shape; pos = torch.arange(seq_len, device=input_ids.device).unsqueeze(0)
        x = self.token_emb(input_ids) + self.pos_emb(pos)
        causal = torch.triu(torch.ones(seq_len, seq_len, device=input_ids.device, dtype=torch.bool), diagonal=1)
        return self.lm_head(self.norm(self.transformer(x, mask=causal)))
